...
def gaussian_likelihood(x, mu, log_std):
    ...
    """
    sigma = tf.reduce_sum(((x-mu)**2/tf.exp(2*log_std) + 2*log_std), axis=1)
    single_likelihood = -(sigma + dim * tf.log(2*tf.constant(np.pi)))/2
...
if __name__ == '__main__':
    ...
    """
    from problem_set_1_solutions import exercise1_1_soln
